...
class PyMap:
    ...
    def __init__(self):
        ...
        if len(self.Arguments['address']) is None:
        ...
        self.PortList = self.PortTokenizer(self.Arguments['port'])


        # Check if the predefined port lists are being used, otherwise use the port argument.
        self.Check_Predefined_Scan_Modes()

        if self.IsTargetValid(target):
            # Scan here
            self.ProcessScans(target)
        else:
            print("Address error, terminating scan.")

    def Check_Predefined_Scan_Modes(self):
        if (self.Arguments['tcptop1k']):
            self.PortList = self.PortTokenizer(self.Top1kTCP)
            self.Arguments['mode'] = "TCP"
            print("Scanning top 1000 TCP ports.")
        elif (self.Arguments['udptop1k']):
            self.PortList = self.PortTokenizer(self.Top1kUDP)
            self.Arguments['mode'] = "UDP"
            print("Scanning top 1000 UDP ports.")
        elif (self.Arguments['tcpall']):
            self.PortList = self.PortTokenizer("1-65535")
            self.Arguments['mode'] = "TCP"
            print("Scanning all TCP ports.")
        elif (self.Arguments['udpall']):
            self.PortList = self.PortTokenizer("1-65535")
            self.Arguments['mode'] = "UDP"
            print("Scanning all UDP ports.")
        elif (self.Arguments['web']):
            self.PortList = self.PortTokenizer("80,443,8080,8443")
            self.Arguments['mode'] = "TCP"
            print("Scanning all web ports (80, 443, 8080, 8443).")
    ...
    def IsTargetValid(self, item: str) -> bool:
        ...
        if result:
            ...
            print(
                f"Unable to identify target(s): \'{item}\' based on supplied address format. Please supply the target as the last parameter using standard notation.")
    ...
    #
    def ProcessScans(self, address: str):
        addressesToProcess = []
        if '/' in address:
        ...
        try:
            ...
            while (startAddress != endAddress):
                if startAddress != int(networkBin, 2):
                    ...
                    resultSet.append(".".join(map(str, [int(binAddress[0:8], 2), int(binAddress[8:16], 2),
                                                        int(binAddress[16:24], 2), int(binAddress[24:32], 2)])))
                startAddress += 1
    ...
    def PortTokenizer(self, ports: str) -> list[int]:
        ...
        if ports.count(',') > 0:
            ...
            try:
                print(f"about to create an integer from {ports}")
                port = int(ports, 10)
    ...
    def SetupArguments(self):
        self.Parser.add_argument('address', metavar='Address', type=str,
                                 help="The hostname or IP address of the target. Also supports subnet ranges using CIDR notation.")
        self.Parser.add_argument("-p", "--port", dest="port", default="80",
                                 help="Default: 80. The Port(s) to scan. Can be comma delimited hyphenated. Use - to scan all ports.")
        self.Parser.add_argument("-m", "--mode", dest="mode", choices=['TCP', 'UDP'], default="TCP",
                                 help="Default: TCP. The protocol used during the scan (e.g. TCP, UDP.)")
        self.Parser.add_argument("-w", "--wait", dest="timeout", default=50, type=int,
                                 help="Default: 50. The timeout in ms to wait for connections. Increase to improve accuracy, but will slow scan speed.")
        self.Parser.add_argument("-v", "--version", dest="version", action="version",
                                 version=f'%(prog)s (version {version})', help="Displays the current version of PyMap.")
        self.Parser.add_argument("-t", "--threads", dest="threads", default=8, type=int,
                                 help="The number of threads to use during the scan. Increase to speed up scans.")
        predefined = self.Parser.add_argument_group(title="Predefined Port Lists",
                                                    description="Use these options to scan a predefined list of ports.")
        predefined.add_argument("--tcp-top-1000", dest="tcptop1k", action="store_true", help="Top 1000 TCP ports.")
        predefined.add_argument("--udp-top-1000", dest="udptop1k", action="store_true", help="Top 1000 UDP ports.")
        predefined.add_argument("--tcp-all", dest="tcpall", action="store_true", help="All TCP ports.")
        predefined.add_argument("--udp-all", dest="udpall", action="store_true", help="All UDP ports.")
        predefined.add_argument("--web", dest="web", action="store_true",
                                help="All ports commonly used for web services (80, 443, 8080, 8443).")
